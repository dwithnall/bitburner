export async function main(ns) {
	try {
				...
				if (factions.includes(faction.name)) {
					...
					if (
						ns.getFactionFavor(faction.name) +
							...
							150 &&
						ns.getFactionFavor(faction.name) < 150
					) {
						while (
							ns.getAugmentationCost(neuroFlux)[1] <=
								ns.getServerMoneyAvailable("home") &&
							ns.getAugmentationCost(neuroFlux)[0] <=
								ns.getFactionRep(faction.name)
						) {
							ns.purchaseAugmentation(faction.name, neuroFlux);
							ns.tprint(
								"Purchasing aug from " + faction.name + ": " + neuroFlux
							);
							await ns.sleep(100);
						}
						if (
					...
					for (var a in factionAugs) {
						...
						if (!ns.getOwnedAugmentations(true).includes(aug)) {
							...
							if (repCost <= ns.getFactionRep(faction.name)) {
								...
								while (
									ns.getFactionRep(faction.name) < repCost &&
									ns.getFactionFavor(faction.name) < 150
								) {
		...
		ns.tprint("Don't have getCharacterInformation  yet");
	}
}
